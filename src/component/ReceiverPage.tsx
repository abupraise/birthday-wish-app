...
export default function ReceiverPage() {
    ...
    // Load wish data from localStorage
    useEffect(() => {
        try {
            ...
            if (data) {
                ...
                console.error("No data found in localStorage. Using default.");
                setWishData({ celebrantName: "Friend", celebrantAge: "Unknown", senderName: "Anonymous", message: "Best wishes!" });
            }
        } catch (error) {
        ...
        setLoading(false); // Remove loading timeout for faster load
    }, [id]);

    // Start listening for blowing sound
    useEffect(() => {
        if (isListening) {
            startListening();
        } else {
    ...
    const startListening = async () => {
        try {
            ...
            console.error("Microphone access denied. Fallback to manual blow out.");
            setCandlesBlownOut(true); // Fallback to manual blow out
        }
    };
